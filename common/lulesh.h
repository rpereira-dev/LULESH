...
#if TRACE
// # define TRACE_CONDITION (myRank == 0 || myRank == numRanks / 2 || myRank == numRanks - 1 || myRank == numRanks / 5 * 1 || myRank == numRanks / 5     * 2 || myRank == numRanks / 5 * 3 || myRank == numRanks / 5 * 4)
//  # define TRACE_CONDITION (myRank % 7 == 0)
// # define TRACE_CONDITION (myRank == 49) // myRank == numRanks / 2 + 1)
# define TRACE_CONDITION (myRank == 0)
// # define TRACE_CONDITION (myRank == 82)
#endif
...
# define PRIORITY_SEND      INT_MAX
# define PRIORITY_REDUCE    INT_MAX
...
class Domain {
    ...
    public:
        ...
        Domain(Int_t numRanks, Index_t colLoc,
                ...
                Int_t te, Int_t tn, Int_t rpf, Int_t rpe);
        ...
        unsigned int delvsize(void) { return m_delv.size(); }
...
#if USE_MPI
        std::vector<communication_partite_t> m_comm[MSG_MAX];
        MPI_Comm m_mpi_comm[MSG_MAX];
        Real_t * m_rbuffer;
        Real_t * m_sbuffer;
#endif
    ...
    private:
        ...
        void SetupCommBuffers(void);
        void SetupCommBuffersForMsg(int msgType);
    ...
    public:
        ...
        // Used in setup to determine neighbor existance
        Index_t m_hasFront, m_hasBack;
        Index_t m_hasLeft, m_hasRight;
        Index_t m_hasBottom, m_hasTop ;

        Int_t m_tel;    /* number of tasks per element-wise loops */
        Int_t m_tnl;    /* number of tasks per node-wise loops */
        Int_t m_ebs;    /* element block size */
        Int_t m_nbs;    /* node block size */
        Int_t m_rpf;    /* number of requests per face communications */
        Int_t m_npfr;   /* number of nodes per face requests */
        Int_t m_rpe;    /* number of requests per edge communications */
        Int_t m_nper;   /* number of nodes per edge requests */
};
...
struct cmdLineOpts {
    ...
    Int_t tel; // -tel
    Int_t tnl; // -tnl
    Int_t rpf; // -rpf // number of requests per face communication
    Int_t rpe; // -rpe // number of requests per edge communication
    Int_t persistentTasks; // -persistent to enable persistent tasks
};
...
// lulesh-comm
#if USE_MPI
void CommGetMsgSpecifics(
    Domain * domain, int msgType,
    Domain_member fields[MAX_FIELDS_PER_MPI_COMM], int & nfields,
    bool & planeOnly,
    Index_t & dx, Index_t & dy, Index_t & dz);
void CommPack  (Domain * domain, int msgType);
void CommUnpack(Domain * domain, int msgType);
void CommRecv(Domain * domain, int msgType);
void CommSend(Domain * domain, int msgType);
void CommReduceDt(Real_t * gnewdt, Real_t * newdt);
void CommDumpTime(void);
#endif /* USE_MPI */
